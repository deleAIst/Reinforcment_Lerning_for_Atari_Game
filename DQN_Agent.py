...
class Agent:
    def __init__(self, game: str):
        ...
        self.optimizer =optim.RMSprop(self.dqn.parameters(), lr=0.00025, eps=0.01)
            ...
            while True:
                ...
                if done:
                    ...
                    if(curremt_reward_mean > best_reward_mean):
                        ...
                        torch.save(self.target_dqn.state_dict(), TARGET_DQN_PATH)
    ...
    def replay(self):
        ...
        q_values_next = self.target_dqn(next_states)
        ...
        for i in range(self.batch_size):
            ...
            if done:
                ...
                q_values[i][a] =  rewards[i]+ self.gamma * torch.max(q_values_next[i]).item()
                loss = torch.nn.SmoothL1Loss(states, q_values)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
    ...
    def play(self, num_episodes, render= True):
        ...
        for episode in self.range(1, num_episodes +1):
            ...
            state = self.env.reset()
            #state = np.reshape(state, newshape=(1, -1)).astype(np.float32)
            state = torch.tensor(data=state, dtype=torch.float32, device=self.device)
            while True:
                if render:
                    self.env.render()
                action = self.get_action(state)
